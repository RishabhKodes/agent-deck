...
the base is now 32 ms, yielding a 160 ms local gate through `ColdBudget` (and
320 ms when CI explicitly uses `PERF_BUDGET_MULTIPLIER=2`).
...
run, competing package tests raised the n=11 trimmed mean to 88.8–146.9 ms even
...
The 160 ms upper bound leaves roughly 9% headroom above the highest observed
